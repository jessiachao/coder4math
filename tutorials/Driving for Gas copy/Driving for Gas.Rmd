---
title: "Driving for Gas (testing on 2/24)"
tutorial: 
  id: "test.id.1"
  version: 0.5
...
# Data sets
gas <- read.csv(file="https://docs.google.com/uc?id=1zJ7NHrXJWW1OvUTs08WA0_X9yCxf8MoP&export=download", sep=",", header=T, stringsAsFactors = FALSE)
...
In this activity, you will use mathematical modeling and R programming skills to build a model
...
gas <- read.csv("~/Google Drive/CodeR4MATH/2.1 Curriculum/Lifehacking/V4_WHS_03.2019/Driving for Gas_working/Driving for Gas/Driving_for_Gas_files/data sets/gas stations - 6 vars.csv", stringsAsFactors=FALSE)
...
library(dplyr)
arrange(gas, price, distance)
```


```{r gas-price, exercise = TRUE}
color = rgb(red = 1, green = 0, blue = 0, alpha = 0.5)
plot(x = gas$price, y = gas$distance, pch = 16, col = color)
text(x = gas$price, y = gas$distance, labels = gas$station, pos = 3, offset = 0.3, cex = 0.5, col = "deepskyblue")
```
